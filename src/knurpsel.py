...
class NodeDatabase ():
	...
	def update_request_counts (self, request_counts):
		for (key, request_count) in request_counts.items ():
			self.nodes[key].request_count = request_count
...
class ReportController ():
	...
	def update_request_counts_post (self, request):
		ct = request.headers['Content-Type']

		if ct == 'application/json':
			nodes = request.json
			self.nodes.update_request_counts (nodes)
	...
	def index_post (self, request):
		...
		if ct == 'application/json':
			...
			self.nodes.update_node (json_obj)
			r = Response ("thanks for reporting your status!", status=200)
		else:
			r = Response ("unexpected Content-Type: " + ct)
		return r
...
def respond_report_json ():
	...
	return c.index_get_json (request)
...
@app.route ("/update_request_counts", methods=['POST'])
def respond_update_request_counts ():
	print "called"
	c = app.controllers['report']
	return c.update_request_counts_post (request)
